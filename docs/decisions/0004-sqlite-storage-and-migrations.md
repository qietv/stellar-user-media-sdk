...
2026-09-03 修订：`library.sqlite` 通过 checksum 固定的 v2/v3/v4/v5/v6/v7/v8/v9/v10 migration 增加
`scan_frontier`、`scan_seen`、`scan_discovery`、revision-safe worker lease 与 claim 顺序索引，并强制单来源只有一个 active scan run；
v6 以事务级 revision 取代 PosterWall 的逐行整库哈希，v7 让终态失败退出热 claim 索引，
v8/v9 增加复合媒体与片单缩略图，v10 增加 missing retention、offline overlay 和两阶段 metadata GC；
当前为 32 张表，v1 的 27 表 DDL 继续作为迁移基线保留。
...
- `library.sqlite` 当前保存 32 张核心业务表；`metadata_cache.sqlite` 只保存 3 张可删除缓存表；`account.sqlite` 保存来源配置、v1 明文 `CredentialRecord`、冲突候选、transactional outbox 与同步 cursor。凭据保护决策以后续 [ADR-0005](0005-synced-credential-storage.md) 为准。
