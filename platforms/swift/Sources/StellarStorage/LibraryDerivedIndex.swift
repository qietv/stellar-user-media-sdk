...
extension LibraryStore {
  ...
  /// Upserts one disposable search document inside an existing library transaction.
  static func upsertSearchDocument(
    entityID: Int64,
    ...
    try database.execute(
      sql: """
        ...
        SELECT e.id,
               ...
               ?
        FROM media_entity e
        WHERE e.id = ? AND e.deleted_at_ms IS NULL AND e.status != 'deleted'
        ...
        """,
      arguments: [updatedAtMilliseconds, entityID]
    )
  }
}
