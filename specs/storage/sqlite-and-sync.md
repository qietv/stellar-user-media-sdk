...
当前 `library.sqlite` 版本为 v11；[`schema-migrations.json`](schema-migrations.json) 固定
...
[`library-v11.sql`](sql/library-v11.sql) 的 checksum。
...
实体垃圾回收状态，v11 增加 revision-safe 的复合媒体深度探测缓存；现行媒体库共 33 张业务表。
...
- `library.sqlite`：33 张媒体库核心表，包括扫描 frontier/seen/discovery staging、文件、复合媒体 probe 缓存、影视实体、片单缩略图、用户状态、媒体库 revision 和同步状态。
