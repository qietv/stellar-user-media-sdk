...
本模块同步“如何连接和扫描媒体源”的配置，包括 NAS、网盘、媒体服务器和本地目录。用户名、密码、Cookie、第三方 refresh token、私钥等通过独立的 `CredentialRecord` 同步；v1 使用应用层明文且不要求设备批准或 Vault 解锁，规则见 [`../security/credential-storage.md`](../security/credential-storage.md)。
...
| `credential_mode` | enum | `synced`、`device_local`、`server_managed`、`none` |
...
- `none` 只用于无需认证的来源。
- MediaSourceConfig 和 CredentialRecord 使用独立 revision/outbox；任一到达顺序都必须安全。缺少配置或凭据记录时不得尝试连接。
